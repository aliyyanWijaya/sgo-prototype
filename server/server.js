...
app.post('/api/aroha-chat', async (req, res) => {
  ...
  try {
    ...
    const reply = textBlock?.text ?? "Ka aroha! I wasn't sure how to respond to that. Could you try asking in a different way?";

    res.json({ reply });
  } catch (err) {
    console.error('Anthropic API error:', err?.message ?? err);
    res.status(502).json({ error: 'upstream_error', message: 'Could not reach the AI service.' });
  }
});
