...
export function UserPreferencesProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const keys = Object.entries(STORAGE_KEYS) as [keyof UserPreferences, string][];
    Promise.all(keys.map(([k, storageKey]) => AsyncStorage.getItem(storageKey).then((v) => [k, v] as const)))
      .then((entries) => {
        setPrefs((prev) => {
          const next = { ...prev };
  ...
  return (
    <UserPreferencesContext.Provider value={{ ...prefs, loaded, setPreference, scale }}>
      {children}
